...
export default class PlayMusic extends Command {
	async run(ctx: CommandContext<typeof option>) {
		...
		try {
			...
			ctx.client.logger.warn(`ERROR: ${err}`)
		...
		ctx.client.logger.debug(`now Voice Room ID : ${voiceChannelId}`) // 🛠 Debug log

		try {
			// 🎶 Load tracks from URL or search query
			const result = await loadTracks(query, member)
			ctx.client.logger.debug(`result : ${result}`)
			...
			ctx.client.logger.debug(`TRACK: ${[track]}`)
